...
#             y = df_id.loc[:,self.features['target']]
#             msk = df_id.loc[:,'msk']
#             dt = df_id.loc[:,['t0','t1']]
#             msk0 = df_id.loc[:,'msk0']
#             Xt = np.array(Xt).astype(np.float32)
...
#     def __init__(self,features,df_train,df_test,batch_size = 64, verbose = False):
...
#         # preprocess
#         self.preproc = PreProcessSim(['glucose_t_obs'],['target'],QuantileTransformer(),QuantileTransformer())
